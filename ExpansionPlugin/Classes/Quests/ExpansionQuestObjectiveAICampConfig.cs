...
namespace ExpansionPlugin
{
    public class ExpansionQuestObjectiveAICampConfig : ExpansionQuestObjectiveConfig
    {
        ...
        internal override void createDefault()
        {
            ...
            string path = Path.Combine(AppServices.GetRequired<ExpansionManager>()._paths["ExpansionQuestObjectives"], "AICamp", $"Objective_AIC_{ID}.json");

            ObjectiveText = "New Ai Camp";
            InfectedDeletionRadius = 500m;
            MaxDistance = -1;
            MinDistance = -1;
            AllowedWeapons = new BindingList<string>();
            AllowedDamageZones = new BindingList<string>();
