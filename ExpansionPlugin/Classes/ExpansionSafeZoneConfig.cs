...
{
    public class ExpansionSafeZoneConfig : IConfigLoader
    {
        private readonly string _path;
        public string FileName => Path.GetFileName(_path); // e.g., "types.xml"
        public string FilePath => _path;
        public ExpansionSafeZoneSettings Data { get; private set; }
        public bool HasErrors { get; private set; }
        public List<string> Errors { get; private set; } = new List<string>();
        public bool isDirty { get; set; }
        public const int CurrentVersion = 11;

        public ExpansionSafeZoneConfig(string path)
        {
            _path = path;
        }
        public void Load()
        {
            Data = null;
            Data = AppServices.GetRequired<FileService>().LoadOrCreateJson<ExpansionSafeZoneSettings>(
                _path,
                createNew: () => new ExpansionSafeZoneSettings(CurrentVersion),
                onAfterLoad: cfg => { },
                onError: ex =>
                {
                    HasErrors = true;
    ...
    public class ExpansionSafeZoneSettings
    {
        ...
        {
            if (obj is not ExpansionSafeZoneSettings other)
                return false;
