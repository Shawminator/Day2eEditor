...
namespace ExpansionPlugin
{
    public class ExpansionHardlineConfig : IConfigLoader
    {
        ...
        {
            Data.ItemRarity = new Dictionary<string, int>();
            foreach (string item in Data.PoorItems)
            {
                Data.ItemRarity.Add(item, 1);
            }
            foreach (string item in Data.CommonItems)
            {
                Data.ItemRarity.Add(item, 2);
            }
            foreach (string item in Data.UncommonItems)
            {
                Data.ItemRarity.Add(item, 3);
            }
            foreach (string item in Data.RareItems)
            {
                Data.ItemRarity.Add(item, 4);
            }
            foreach (string item in Data.EpicItems)
            {
                Data.ItemRarity.Add(item, 5);
            }
            foreach (string item in Data.LegendaryItems)
            {
                Data.ItemRarity.Add(item, 6);
            }
            foreach (string item in Data.MythicItems)
            {
                Data.ItemRarity.Add(item, 7);
            }
            foreach (string item in Data.ExoticItems)
            {
                Data.ItemRarity.Add(item, 8);
            }
            foreach (string item in Data.QuestItems)
            {
                Data.ItemRarity.Add(item, 9);
            }
            foreach (string item in Data.CollectableItems)
            {
                Data.ItemRarity.Add(item, 10);
            }
            foreach (string item in Data.IngredientItems)
            {
                Data.ItemRarity.Add(item, 11);
            }
        }
        public void ConvertReputationDictionarytolist()
        {
            Data.entityreps = new BindingList<EntityReputationlevels>();
            if (Data.EntityReputation == null)
                Data.EntityReputation = new Dictionary<string, int>();
            foreach (KeyValuePair<string, int> item in Data.EntityReputation)
            {
                string classaname = item.Key;
                int level = item.Value;
                Data.entityreps.Add(new EntityReputationlevels(classaname, level));
            }
        }
        public void convertreplisttodict()
        {
            Data.EntityReputation = new Dictionary<string, int>();
            foreach (EntityReputationlevels item in Data.entityreps)
            {
    ...
    public class ExpansionHardlineSettings
    {
        ...
        { }
        public ExpansionHardlineSettings(int CurrentVersion)
        {
        ...
        public override bool Equals(object obj)
        {
            ...
            return m_Version == other.m_Version &&
                   PoorItemRequirement == other.PoorItemRequirement &&
                   CommonItemRequirement == other.CommonItemRequirement &&
                   UncommonItemRequirement == other.UncommonItemRequirement &&
                   RareItemRequirement == other.RareItemRequirement &&
                   EpicItemRequirement == other.EpicItemRequirement &&
                   LegendaryItemRequirement == other.LegendaryItemRequirement &&
                   MythicItemRequirement == other.MythicItemRequirement &&
                   ExoticItemRequirement == other.ExoticItemRequirement &&
                   ShowHardlineHUD == other.ShowHardlineHUD &&
                   UseReputation == other.UseReputation &&
                   UseFactionReputation == other.UseFactionReputation &&
                   EnableFactionPersistence == other.EnableFactionPersistence &&
                   EnableItemRarity == other.EnableItemRarity &&
                   UseItemRarityOnInventoryIcons == other.UseItemRarityOnInventoryIcons &&
                   UseItemRarityForMarketPurchase == other.UseItemRarityForMarketPurchase &&
                   UseItemRarityForMarketSell == other.UseItemRarityForMarketSell &&
                   MaxReputation == other.MaxReputation &&
                   ReputationLossOnDeath == other.ReputationLossOnDeath &&
                   DefaultItemRarity == other.DefaultItemRarity &&
                   ItemRarityParentSearch == other.ItemRarityParentSearch &&

                   areListsEqual(PoorItems, other.PoorItems) &&
                   areListsEqual(CommonItems, other.CommonItems) &&
                   areListsEqual(UncommonItems, other.UncommonItems) &&
                   areListsEqual(RareItems, other.RareItems) &&
                   areListsEqual(EpicItems, other.EpicItems) &&
                   areListsEqual(LegendaryItems, other.LegendaryItems) &&
                   areListsEqual(MythicItems, other.MythicItems) &&
                   areListsEqual(ExoticItems, other.ExoticItems) &&
                   areListsEqual(QuestItems, other.QuestItems) &&
                   areListsEqual(CollectableItems, other.CollectableItems) &&
        ...
        public List<string> FixMissingOrInvalidFields()
        {
            ...
            if (PoorItemRequirement == null || PoorItemRequirement < 0)
            {
                PoorItemRequirement = 0;
                fixes.Add("Corrected PoorItemRequirement to 0");
            }
            if (CommonItemRequirement == null || CommonItemRequirement < 0)
            {
                CommonItemRequirement = 0;
                fixes.Add("Corrected CommonItemRequirement to 0");
            }
            if (UncommonItemRequirement == null || UncommonItemRequirement < 0)
            {
                UncommonItemRequirement = 1000;
                fixes.Add("Corrected UncommonItemRequirement to 1000");
            }
            if (RareItemRequirement == null || RareItemRequirement < 0)
            {
                RareItemRequirement = 2000;
                fixes.Add("Corrected RareItemRequirement to 2000");
            }
            if (EpicItemRequirement == null || EpicItemRequirement < 0)
            {
                EpicItemRequirement = 3000;
                fixes.Add("Corrected EpicItemRequirement to 3000");
            }
            if (LegendaryItemRequirement == null || LegendaryItemRequirement < 0)
            {
                LegendaryItemRequirement = 4000;
                fixes.Add("Corrected LegendaryItemRequirement to 4000");
            }
            if (MythicItemRequirement == null || MythicItemRequirement < 0)
            {
                MythicItemRequirement = 5000;
                fixes.Add("Corrected MythicItemRequirement to 5000");
            }
            if (ExoticItemRequirement == null || ExoticItemRequirement < 0)
            {
                ExoticItemRequirement = 10000;
                fixes.Add("Corrected ExoticItemRequirement to 10000");
            }
            if (ShowHardlineHUD == null || (ShowHardlineHUD != 0 && ShowHardlineHUD != 1))
            {
                ShowHardlineHUD = 1;
                fixes.Add("Corrected ShowHardlineHUD to 1");
            }
            if (UseReputation == null || (UseReputation != 0 && UseReputation != 1))
            {
                UseReputation = 1;
                fixes.Add("Corrected UseReputation to 1");
            }
            if (UseFactionReputation == null || (UseFactionReputation != 0 && UseFactionReputation != 1))
            {
                UseFactionReputation = 1;
                fixes.Add("Corrected UseFactionReputation to 1");
            }
            if (EnableFactionPersistence == null || (EnableFactionPersistence != 0 && EnableFactionPersistence != 1))
            {
                EnableFactionPersistence = 1;
                fixes.Add("Corrected EnableFactionPersistence to 1");
            }
            if (EnableItemRarity == null || (EnableItemRarity != 0 && EnableItemRarity != 1))
            {
                EnableItemRarity = 1;
                fixes.Add("Corrected EnableItemRarity to 1");
            }
            if (UseItemRarityOnInventoryIcons == null || (UseItemRarityOnInventoryIcons != 0 && UseItemRarityOnInventoryIcons != 1))
            {
                UseItemRarityOnInventoryIcons = 1;
                fixes.Add("Corrected UseItemRarityOnInventoryIcons to 1");
            }
            if (UseItemRarityForMarketPurchase == null || (UseItemRarityForMarketPurchase != 0 && UseItemRarityForMarketPurchase != 1))
            {
                UseItemRarityForMarketPurchase = 1;
                fixes.Add("Corrected UseItemRarityForMarketPurchase to 1");
            }
            if (UseItemRarityForMarketSell == null || (UseItemRarityForMarketSell != 0 && UseItemRarityForMarketSell != 1))
            {
                UseItemRarityForMarketSell = 1;
                fixes.Add("Corrected UseItemRarityForMarketSell to 1");
            }
            if (MaxReputation == null || MaxReputation < 0)
            {
                MaxReputation = 100;
                fixes.Add("Set MaxReputation to 100");
            }
            if (ReputationLossOnDeath == null || ReputationLossOnDeath < 0)
            {
                ReputationLossOnDeath = 100;
                fixes.Add("Set ReputationLossOnDeath to 100");
            }
            if (DefaultItemRarity == null || DefaultItemRarity < 0)
            {
                DefaultItemRarity = 2;
                fixes.Add("Set DefaultItemRarity to 2");
            }
            if (ItemRarityParentSearch == null || ItemRarityParentSearch < 0)
            {
                ItemRarityParentSearch = 0;
                fixes.Add("Set ItemRarityParentSearch to 0");
            }
            ...
            {
                DefaultEntityReputation();
                fixes.Add("Initialized EntityReputation dictionary"); 
            }
            if (ItemRarity == null)
            {
                SetDefaultItemRarity();
                fixes.Add("Initialized ItemRarity dictionary"); 
            }
